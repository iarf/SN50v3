...
__IO bool power_down_command = 0;
...
static void PowerStartPowerDownTimer(void);
static void prepare_sleep_state(void);
...
int main(void)
{
    ...
    // XXX: start power-down timer
    PowerStartPowerDownTimer();
...
static void LORA_RxData( lora_AppData_t *AppData )
{
  ...
  // Only check for command input if the message is sent on Port 1
 ...
 */
static void
OnPowerDownTimerEvent(void)
{

    TimerStop(&PowerDownTimer);	
    power_down_command=1;
}

/*
 * XXX: determine time-out
 */
static void
PowerStartPowerDownTimer(void)
{
    TimerInit(&PowerDownTimer, OnPowerDownTimerEvent );
    TimerSetValue(&PowerDownTimer, PowerDownTimerPeriod); 
    TimerStart(&PowerDownTimer);	
}
